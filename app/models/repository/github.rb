...
class Repository::Github < Repository
  ...
  def set_root_url
    if !url.blank? && root_url.blank?
      uri = URI.parse(url)
      uri.path = ''
      self.root_url = uri.to_s
    end
    self.root_url = root_url.to_s.chomp('/')
  end
  ...
  def default_branch
    ...
    logger.error "github: error during get default branch: #{e.message}"
    nil
  end
  ...
  def save_revisions(prev_db_heads, repo_heads, last_committed_date)
    ...
    limit = 100
    offset = 0
    revisions_copy = revisions.clone
